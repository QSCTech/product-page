...
const useIntersectionObserver = (setActiveId:React.Dispatch<React.SetStateAction<string>>) => {
  const partElementsRef: React.MutableRefObject<{[id:string]:IntersectionObserverEntry}> = useRef({});

  useEffect(() => {
    const callback:IntersectionObserverCallback = (theList: IntersectionObserverEntry[]) => {
      partElementsRef.current = theList.reduce((map:{[id:string]:IntersectionObserverEntry}, partElement: IntersectionObserverEntry) => {
...
export default function Outliner(): JSX.Element {
  ...
  function scrollTo(selector: string){// smoothly scroll
    return (
      (e: React.MouseEvent) => {
        e.preventDefault();
  ...
  return (<>
    <nav className={styles.outliner} aria-label="Outliner or Table Of Content">
      <ul>
